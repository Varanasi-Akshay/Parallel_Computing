...
For instance, the range between \n{do} and \n{end do} is not a scope.
...
in modern Fortran, you can add the \indextermfort{Save} specification
to a variable declaration.

Another mechanism for creating static data was the
\indextermfort{Common} block. This should not be used, since a
\indextermfort{Module} is a more elegant solution to the same problem.

\Level 1 {Variables in an internal procedure}

An \indextermsub{internal}{procedure} (that is, one placed in the
\indextermfort{Contains} part of a program unit) can receive arguments
