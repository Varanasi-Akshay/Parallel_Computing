...
#ifdef TRACE
	if (procno==0)
	  printf("wait 1 for step=%d\n",step);
#endif
	MPI_Waitall(8,requests1,MPI_STATUSES_IGNORE);
      }
      /*
       * barrier so that we know 1 has arrived
       */
#pragma omp barrier
      // ================================================
      /*
       * we can now update 1,
       * and pack for the next 1,
       * wait for 2
       */
      if (thread_num==1) {
#include "update1.c"
...
#ifdef TRACE
	if (procno==0)
	  printf("wait 2 for step=%d\n",step);
#endif
	MPI_Waitall(8,requests2,MPI_STATUSES_IGNORE);
      }

      /*
       * barrier so that we know 2 has arrived
       * after this, update 2, pack for the next 2, wait for 1
       */
#pragma omp barrier
      // ================================================
      if (thread_num==1) {
#include "update2.c"
  ...
  if (procno==0)
    printf("overlap4h runtime %e for %e flops\n",runtime,flops);
