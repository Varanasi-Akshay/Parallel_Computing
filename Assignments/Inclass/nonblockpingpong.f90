...
integer ::status(MPI_STATUS_SIZE),nranks,rank,ierr,irec=-1,request
...
if(rank==0) then
   ...
   call mpi_Isend(ping,1,MPI_int,right,right,comm,request,ierr)
   ...
   ping=pong+1
   call mpi_Isend(ping,1,MPI_int,0,0,comm,request,ierr)
